...
class NPPMIntegrator(PPMIntegrator):
    def __init__(self, network: torch.nn.Module, photons_per_iter: int, n_iterations: int, 
                 ...
                 pure_caustic: bool=True, knn_mode: bool=False, use_ray_diff: bool=True, build_guiding: int=None, ratio_matches: bool=False, gaussian: bool = False,
        ...
        super().__init__(photons_per_iter, n_iterations, 
                         device, init_radius, start_iter, n_split, apam, stochastic, seed, pure_caustic, knn_mode, use_ray_diff, build_guiding)
            ...
            for i in tqdm(range(self.start_iter + 1, self.start_iter + self.n_iterations + 1), desc=f"Running NPPM ({"beta" if self.use_cppm_scheme else "alpha"})"):
                ...
                for j in range(NB_SELEC):
                    ...
                    if self.gaussian:
                        ...
                        pred_flux, raw_flux, visible_dir_samples, _ = frnn.frnn_grid_photon_gauss_gather(points[None], normals_photons[None], wo_photons[None], roughness_photons[None],
