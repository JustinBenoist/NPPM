...
def main():
    ...
    metric_names = {'L1': 1, 'L2': 2, 'SMAPE': 3, 'RELMSE': 4}
            ...
            for test in all_tests:
                ...
                for file in os.listdir(test_path):
                    ...
                    try:
                        label = f"{test}"
                        try:
                            times = [x[0] for x in errors]
                            # Support for (time, (L1, L2, ...)) or (time, L1, L2, ...)
                            y_vals = [x[error_idx] for x in errors]

                            # Convert to floats if needed (torch tensors)
                            times = [t.item() if hasattr(t, "item") else t for t in times]
                            y_vals = [v.item() if hasattr(v, "item") else v for v in y_vals]

                            label = f"{test}"
                            line, = ax.loglog(times, y_vals, label=label, color=color)
                            legend[label] = line
                        except Exception as e:
